...
- [x] 2026-04-14 Added a release-polish integration pass with fixture macOS apps, browser bridge protocol coverage, and repo-local automation.
- [x] 2026-04-14 Fixed stale web-domain state by clearing domains on browser switches and adding a browser bridge `domain_cleared` message.
- [x] 2026-04-14 Verified the new integration pass with `just integration`, `just test-app`, `uv run pytest tests -v`, `just lint`, and `just ci`.
...
- Observation: Web adapter state can become stale if a supported Chrome-family tab reports a domain and the user then switches to another browser or navigates to an unsupported domain that the extension intentionally does not report.
  Evidence: `AppMonitor` only clears `webAppDomain` when switching to a non-browser bundle, and `BrowserBridge.decodeMessagePayload` rejects unsupported domains without a way to clear the current web adapter.
...
- Decision: Keep release tooling repo-local and explicit.
  ...
  Date/Author: 2026-04-14 / Codex
- Decision: Add a repo-local macOS automation fixture instead of depending on installed third-party apps for integration coverage.
  Rationale: A small compiled AppKit fixture gives deterministic menus, bundle IDs, activation behavior, and Accessibility probes from a clean checkout.
  Date/Author: 2026-04-14 / Codex
...
The current release-polish pass adds integration coverage beyond unit tests:
fixture macOS apps under `tests/fixtures/`, automation launched through `just
integration`, direct Unix socket browser-bridge tests, and stale web-domain
regression tests.

The release-polish pass also fixed stale browser context. The extension now
sends `domain_cleared` when the active page is unsupported, `BrowserBridge`
clears `AppMonitor.webAppDomain` after that message, and `AppMonitor` clears
web state when activation moves to a different browser or non-browser app.

## Context and Orientation

The repository root for this pass is `/Users/peyton/.codex/worktrees/435b/shorty`. `justfile` is the primary command surface. `mise.toml` pins tools. `pyproject.toml` and `uv.lock` define Python validation dependencies. The macOS app is under `app/` and is generated by Tuist from `app/Shorty/Project.swift`. The static public website is under `web/`. Python tooling lives under `scripts/` and tests under `tests/`.
...
For the release-polish integration pass, add a small AppKit fixture app that
builds into `.build/fixtures/`, exposes deterministic menu items, and can be
launched and inspected by a repo-local macOS automation probe. Add browser
bridge socket tests that send real length-prefixed frames over a Unix domain
socket. Add stale-domain regression coverage so unsupported navigation and
browser switches clear the effective web adapter. Wire the new coverage through
`just integration` and include it in `just ci` while keeping permission-gated UI
scripting optional unless `SHORTY_REQUIRE_UI_AUTOMATION=1` is set.

## Concrete Steps

Work from the repository root:

    cd /Users/peyton/.codex/worktrees/435b/shorty
